...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Безумный Макс: Дорога ярости"'

    def handle(self, *args, **options):
        self.stdout.write("🚗 ОБНОВЛЕНИЕ ПОСТЕРА 'БЕЗУМНЫЙ МАКС: ДОРОГА ЯРОСТИ'")
        self.stdout.write("=" * 60)
        
        # URL постера с КиноПоиск (Яндекс) - заменим размер на более крупный
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/1600647/4e7bd7aa-50f9-4e73-b086-6d725ed5a035/orig'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Безумный Макс: Дорога ярости')
            ...
            # Сохраняем постер
            filename = "mad_max_fury_road_2015_kinopoisk.jpg"
            ...
            self.stdout.write(f"🎭 Главная роль: Том Харди")
            self.stdout.write(f"🎬 Режиссер: Джордж Миллер")
            self.stdout.write(f"🏆 6 Оскаров")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Безумный Макс: Дорога ярости" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='безумный'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='макс'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='mad max'):
                self.stdout.write(f"  - {film.title} ({film.year})")
