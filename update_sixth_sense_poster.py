...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Шестое чувство"'

    def handle(self, *args, **options):
        self.stdout.write("👻 ОБНОВЛЕНИЕ ПОСТЕРА 'ШЕСТОЕ ЧУВСТВО'")
        self.stdout.write("=" * 50)
        
        # URL постера с КиноПоиск (Яндекс)
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/1600647/a99e9d0e-32e1-4d13-961e-e813b1c5c2dc/orig'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Шестое чувство')
            ...
            # Сохраняем постер
            filename = "sixth_sense_1999_kinopoisk.jpg"
            ...
            self.stdout.write(f"🎬 Режиссер: М. Найт Шьямалан")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Шестое чувство" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='шестое'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='чувство'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='sixth sense'):
                self.stdout.write(f"  - {film.title} ({film.year})")
