...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Птицы"'

    def handle(self, *args, **options):
        self.stdout.write("🐦 ОБНОВЛЕНИЕ ПОСТЕРА 'ПТИЦЫ'")
        self.stdout.write("=" * 40)
        
        # URL постера с КиноПоиск (Яндекс) - заменим размер на более крупный
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/1773646/cb6bf320-fdbf-499d-b927-11bf139cd596/orig'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Птицы')
            ...
            # Сохраняем постер
            filename = "birds_1963_kinopoisk.jpg"
            ...
            self.stdout.write(f"🐦 Классика хоррора")
            self.stdout.write(f"🏆 Мастер саспенса")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Птицы" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='птицы'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='birds'):
                self.stdout.write(f"  - {film.title} ({film.year})")
