...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "В поисках Немо"'

    def handle(self, *args, **options):
        self.stdout.write("🐠 ОБНОВЛЕНИЕ ПОСТЕРА 'В ПОИСКАХ НЕМО'")
        self.stdout.write("=" * 50)
        
        # URL постера с Film.ru
        poster_url = 'https://www.film.ru/sites/default/files/styles/thumb_260x400/public/movies/posters/1610708-1580929.jpeg'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='В поисках Немо')
            self.stdout.write(f"🎬 Найден фильм: {film.title} ({film.year})")
            
            # Загружаем новый постер
            self.stdout.write("📥 Загружаю постер с Film.ru...")
            ...
            # Сохраняем постер
            filename = "finding_nemo_2003_filmru.jpg"
            ...
            self.stdout.write(f"🎭 Студия: Pixar Animation Studios")
            self.stdout.write(f"🏆 Оскар за лучший анимационный фильм")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "В поисках Немо" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='немо'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='поисках'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='finding nemo'):
                self.stdout.write(f"  - {film.title} ({film.year})")
