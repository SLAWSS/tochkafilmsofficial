...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Фарго"'

    def handle(self, *args, **options):
        self.stdout.write("❄️ ОБНОВЛЕНИЕ ПОСТЕРА 'ФАРГО'")
        self.stdout.write("=" * 40)
        
        # URL постера с Google Images
        poster_url = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTynW43uPx6VWdMe2M80ULzgyO9R4n_QdSNpA&s'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Фарго')
            ...
            # Сохраняем постер
            filename = "fargo_1996_google.jpg"
            ...
            self.stdout.write(f"🏆 Оскар за лучший оригинальный сценарий")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Фарго" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='фарго'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='fargo'):
                self.stdout.write(f"  - {film.title} ({film.year})")
