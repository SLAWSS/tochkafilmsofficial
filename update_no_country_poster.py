...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Старикам тут не место"'

    def handle(self, *args, **options):
        self.stdout.write("🤠 ОБНОВЛЕНИЕ ПОСТЕРА 'СТАРИКАМ ТУТ НЕ МЕСТО'")
        self.stdout.write("=" * 55)
        
        # URL постера с Ананас Постер
        poster_url = 'https://cdn.ananasposter.ru/image/cache/catalog/poster/film/76/45633-1000x830.jpg'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Старикам тут не место')
            ...
            # Сохраняем постер
            filename = "no_country_for_old_men_2007_ananas.jpg"
            ...
            self.stdout.write(f"🎭 Главная роль: Томми Ли Джонс")
            self.stdout.write(f"🎬 Режиссеры: Братья Коэн")
            self.stdout.write(f"🏆 Оскар за лучший фильм")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Старикам тут не место" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='старикам'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='место'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='no country'):
                self.stdout.write(f"  - {film.title} ({film.year})")
