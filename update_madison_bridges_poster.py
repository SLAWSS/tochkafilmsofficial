...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Мосты округа Мэдисон"'

    def handle(self, *args, **options):
        self.stdout.write("🌉 ОБНОВЛЕНИЕ ПОСТЕРА 'МОСТЫ ОКРУГА МЭДИСОН'")
        self.stdout.write("=" * 50)
        
        # URL постера с Кино-Театр.ру
        poster_url = 'https://www.kino-teatr.ru/movie/posters/big/2/4/25742.jpg'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Мосты округа Мэдисон')
            ...
            # Сохраняем постер
            filename = "madison_bridges_1995_kinoteatr.jpg"
            ...
            self.stdout.write(f"🎬 Режиссер: Клинт Иствуд")
            self.stdout.write(f"💕 Мерил Стрип и Клинт Иствуд")
            self.stdout.write(f"🌉 Романтическая драма")
            self.stdout.write(f"📖 По роману Роберта Джеймса Уоллера")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Мосты округа Мэдисон" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='мост'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='мэдисон'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='madison'):
                self.stdout.write(f"  - {film.title} ({film.year})")
