...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Рассвет мертвецов"'

    def handle(self, *args, **options):
        self.stdout.write("🧟 ОБНОВЛЕНИЕ ПОСТЕРА 'РАССВЕТ МЕРТВЕЦОВ'")
        self.stdout.write("=" * 50)
        
        # URL постера с КиноПоиск (Яндекс) - заменим размер на более крупный
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/1704946/98609f73-120e-4612-a553-a386faf48230/orig'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Рассвет мертвецов')
            ...
            # Сохраняем постер
            filename = "dawn_of_dead_1978_kinopoisk.jpg"
            ...
            self.stdout.write(f"🧟 Классика зомби-хоррора")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Рассвет мертвецов" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='рассвет'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='мертвецов'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='dawn'):
                self.stdout.write(f"  - {film.title} ({film.year})")
