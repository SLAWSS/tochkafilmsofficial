...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Головокружение"'

    def handle(self, *args, **options):
        self.stdout.write("🌀 ОБНОВЛЕНИЕ ПОСТЕРА 'ГОЛОВОКРУЖЕНИЕ'")
        self.stdout.write("=" * 50)
        
        # URL постера с КиноХод
        poster_url = 'https://kinohod.ru/o/f5/1b/f51b0717-7e30-4c76-942a-382a7dc4baa3.jpg'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Головокружение')
            self.stdout.write(f"🎬 Найден фильм: {film.title} ({film.year})")
            
            # Загружаем новый постер
            self.stdout.write("📥 Загружаю постер с КиноХод...")
            ...
            # Сохраняем постер
            filename = "vertigo_1958_kinohod.jpg"
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Головокружение" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='головокружение'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='vertigo'):
                self.stdout.write(f"  - {film.title} ({film.year})")
