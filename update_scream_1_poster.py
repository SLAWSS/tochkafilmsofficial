...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Крик"'

    def handle(self, *args, **options):
        self.stdout.write("😱 ОБНОВЛЕНИЕ ПОСТЕРА 'КРИК'")
        self.stdout.write("=" * 35)
        
        # URL постера с Poster4.me
        poster_url = 'https://poster4.me/wp-content/uploads/krik_5-350x527.jpg'
        ...
        try:
            ...
            self.stdout.write("📥 Загружаю постер с Poster4.me...")
            ...
            # Сохраняем постер
            filename = "scream_1996_poster4me.jpg"
            ...
            self.stdout.write(f"🎬 Режиссер: Уэс Крэйвен")
            self.stdout.write(f"👻 Призрачное лицо")
            self.stdout.write(f"📞 Звонки убийцы")
            self.stdout.write(f"🔪 Слэшер 90-х")
            self.stdout.write(f"🎭 Пародия на хоррор")
            self.stdout.write(f"📺 Культовый фильм")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Крик" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='крик'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='scream'):
                self.stdout.write(f"  - {film.title} ({film.year})")
