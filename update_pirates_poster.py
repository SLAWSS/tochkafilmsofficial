...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Пираты Карибского моря"'

    def handle(self, *args, **options):
        self.stdout.write("🏴‍☠️ ОБНОВЛЕНИЕ ПОСТЕРА 'ПИРАТЫ КАРИБСКОГО МОРЯ'")
        self.stdout.write("=" * 60)
        
        # URL постера с КиноПоиск (Яндекс)
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/1773646/d7e3dbd6-e4a9-4485-b751-d02f49825166/orig'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Пираты Карибского моря')
            ...
            # Сохраняем постер
            filename = "pirates_caribbean_2003_kinopoisk.jpg"
            ...
            self.stdout.write(f"🎭 Главная роль: Джонни Депп")
            self.stdout.write(f"🎬 Режиссер: Гор Вербински")
            self.stdout.write(f"🏴‍☠️ Капитан Джек Воробей")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Пираты Карибского моря" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='пираты'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='карибского'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='pirates'):
                self.stdout.write(f"  - {film.title} ({film.year})")
