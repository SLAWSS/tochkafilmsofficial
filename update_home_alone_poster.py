...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Один дома"'

    def handle(self, *args, **options):
        self.stdout.write("🏠 ОБНОВЛЕНИЕ ПОСТЕРА 'ОДИН ДОМА'")
        self.stdout.write("=" * 45)
        
        # URL постера с КиноПоиск (Яндекс) - заменим размер на более крупный
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/1946459/c7195513-24f6-4e80-8f8a-d6777577abb9/orig'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Один дома')
            ...
            # Сохраняем постер
            filename = "home_alone_1990_kinopoisk.jpg"
            ...
            self.stdout.write(f"🎭 Главная роль: Маколей Калкин")
            self.stdout.write(f"🎬 Режиссер: Крис Коламбус")
            self.stdout.write(f"🏠 Рождественская классика")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Один дома" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='один'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='дома'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='home alone'):
                self.stdout.write(f"  - {film.title} ({film.year})")
