...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Игры разума"'

    def handle(self, *args, **options):
        self.stdout.write("🧠 ОБНОВЛЕНИЕ ПОСТЕРА 'ИГРЫ РАЗУМА'")
        self.stdout.write("=" * 50)
        
        # URL постера с Google Images
        poster_url = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTfw4Qn8vBXFbX8z9xitrH-KSEOPgr76mFzgg&s'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Игры разума')
            self.stdout.write(f"🎬 Найден фильм: {film.title} ({film.year})")
            
            # Загружаем новый постер
            self.stdout.write("📥 Загружаю постер...")
            ...
            # Сохраняем постер
            filename = "beautiful_mind_2001_google.jpg"
            ...
            self.stdout.write(f"🎭 Главная роль: Рассел Кроу")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Игры разума" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='игры'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='разума'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='beautiful mind'):
                self.stdout.write(f"  - {film.title} ({film.year})")
