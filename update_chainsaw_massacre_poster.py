...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Резня бензопилой"'

    def handle(self, *args, **options):
        self.stdout.write("🪚 ОБНОВЛЕНИЕ ПОСТЕРА 'РЕЗНЯ БЕНЗОПИЛОЙ'")
        self.stdout.write("=" * 48)
        
        # URL постера с КиноПоиск
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/4483445/59c48d59-c4c6-4f72-af1a-8600e9340d16/180'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Техасская резня бензопилой')
            ...
            # Сохраняем постер
            filename = "chainsaw_massacre_1974_kinopoisk.webp"
            ...
            self.stdout.write(f"🎬 Режиссер: Тоуб Хупер")
            self.stdout.write(f"🪚 Кожаное лицо")
            self.stdout.write(f"😱 Классика слэшера")
            self.stdout.write(f"🏠 Техасская резня")
            self.stdout.write(f"🔪 Культовый хоррор")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Техасская резня бензопилой" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='резня'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='бензопил'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='chainsaw'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='massacre'):
                self.stdout.write(f"  - {film.title} ({film.year})")
