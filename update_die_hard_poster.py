...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Крепкий орешек"'

    def handle(self, *args, **options):
        self.stdout.write("💥 ОБНОВЛЕНИЕ ПОСТЕРА 'КРЕПКИЙ ОРЕШЕК'")
        self.stdout.write("=" * 50)
        
        # URL постера с Википедии (GIF файл)
        poster_url = 'https://upload.wikimedia.org/wikipedia/ru/f/f6/Die_Hard.gif'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Крепкий орешек')
            ...
            # Сохраняем постер (сохраняем как .gif, но Django может конвертировать)
            filename = "die_hard_1988_wikipedia.gif"
            ...
            self.stdout.write(f"🎭 Главная роль: Брюс Уиллис")
                ...
                try:
                    with film.poster.open('rb') as f:
                        ...
                        if header.startswith(b'\xff\xd8\xff'):
                            ...
                            self.stdout.write("ℹ️ Возможно, Django конвертировал формат")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Крепкий орешек" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='крепкий'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='орешек'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='die hard'):
                self.stdout.write(f"  - {film.title} ({film.year})")
