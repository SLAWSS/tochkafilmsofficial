...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Ледниковый период"'

    def handle(self, *args, **options):
        self.stdout.write("🧊 ОБНОВЛЕНИЕ ПОСТЕРА 'ЛЕДНИКОВЫЙ ПЕРИОД'")
        self.stdout.write("=" * 48)
        
        # URL постера с Кино-Театр.ру
        poster_url = 'https://www.kino-teatr.ru/movie/posters/big/8/4/32248.jpg'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Ледниковый период')
            self.stdout.write(f"🎬 Найден фильм: {film.title} ({film.year})")
            
            # Загружаем новый постер
            self.stdout.write("📥 Загружаю постер с Кино-Театр.ру...")
            ...
            # Сохраняем постер
            filename = "ice_age_2002_kinoteatr.jpg"
            ...
            self.stdout.write(f"🎬 Студия: Blue Sky Studios")
            self.stdout.write(f"🦣 Мэнни мамонт")
            self.stdout.write(f"🦥 Сид ленивец")
            self.stdout.write(f"🐅 Диего саблезубый тигр")
            self.stdout.write(f"🧊 Анимационная комедия")
            self.stdout.write(f"👨‍👩‍👧‍👦 Семейный фильм")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Ледниковый период" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='ледник'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='период'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='ice'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='age'):
                self.stdout.write(f"  - {film.title} ({film.year})")
