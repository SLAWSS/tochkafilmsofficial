...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Индиана Джонс и Храм судьбы"'

    def handle(self, *args, **options):
        self.stdout.write("🏛️ ОБНОВЛЕНИЕ ПОСТЕРА 'ИНДИАНА ДЖОНС И ХРАМ СУДЬБЫ'")
        self.stdout.write("=" * 60)
        
        # URL постера с КиноПоиск
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/4486454/e349c47c-4463-41cf-9a1b-484422b4c516/220x330'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Индиана Джонс и храм судьбы')
            ...
            # Сохраняем постер
            filename = "indiana_jones_temple_1984_kinopoisk.webp"
            ...
            self.stdout.write(f"🎬 Режиссер: Стивен Спилберг")
            self.stdout.write(f"🤠 Харрисон Форд")
            self.stdout.write(f"🏛️ Храм Кали")
            self.stdout.write(f"💎 Камни Шанкара")
            self.stdout.write(f"🐍 Приключения в Индии")
            self.stdout.write(f"🎭 Вторая часть трилогии")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Индиана Джонс и храм судьбы" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='индиана'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='джонс'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='храм'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='indiana'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='jones'):
                self.stdout.write(f"  - {film.title} ({film.year})")
