...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Ночь живых мертвецов"'

    def handle(self, *args, **options):
        self.stdout.write("🧟‍♂️ ОБНОВЛЕНИЕ ПОСТЕРА 'НОЧЬ ЖИВЫХ МЕРТВЕЦОВ'")
        self.stdout.write("=" * 60)
        
        # URL постера с КиноПоиск (Яндекс)
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/1898899/24d3cf5c-6657-4797-8ecd-00f8da19ff4f/orig'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Ночь живых мертвецов')
            ...
            # Сохраняем постер
            filename = "night_of_living_dead_1968_kinopoisk.jpg"
            ...
            self.stdout.write(f"🎬 Режиссер: Джордж А. Ромеро")
            self.stdout.write(f"🧟‍♂️ Первый зомби-фильм")
            self.stdout.write(f"🏆 Культовая классика ужасов")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Ночь живых мертвецов" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='ночь'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='мертвецов'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='night'):
                self.stdout.write(f"  - {film.title} ({film.year})")
