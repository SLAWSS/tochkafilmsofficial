...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Бегущий по лезвию"'

    def handle(self, *args, **options):
        self.stdout.write("🤖 ОБНОВЛЕНИЕ ПОСТЕРА 'БЕГУЩИЙ ПО ЛЕЗВИЮ'")
        self.stdout.write("=" * 55)
        
        # URL постера с КиноПоиск (Яндекс) - заменим размер на более крупный
        poster_url = 'https://avatars.mds.yandex.net/get-kinopoisk-image/1898899/fd78b0b9-9ff6-4ca5-8191-8a4173b07283/orig'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Бегущий по лезвию')
            ...
            # Сохраняем постер
            filename = "blade_runner_1982_kinopoisk.jpg"
            ...
            self.stdout.write(f"🎭 Главная роль: Харрисон Форд")
            self.stdout.write(f"🎬 Режиссер: Ридли Скотт")
            self.stdout.write(f"🏆 Культовый фильм киберпанк")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Бегущий по лезвию" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='бегущий'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='лезвию'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='blade runner'):
                self.stdout.write(f"  - {film.title} ({film.year})")
