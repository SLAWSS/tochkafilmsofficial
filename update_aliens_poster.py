...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Чужие"'

    def handle(self, *args, **options):
        self.stdout.write("👽 ОБНОВЛЕНИЕ ПОСТЕРА 'ЧУЖИЕ'")
        self.stdout.write("=" * 45)
        
        # URL постера с Google Images
        poster_url = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTDM_39nriVCkhlS-HyZ-yetn-rdt2Vn4VAog&s'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Чужие')
            ...
            # Сохраняем постер
            filename = "aliens_1986_google.jpg"
            ...
            self.stdout.write(f"🎭 Режиссер: Джеймс Кэмерон")
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Чужие" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='чужи'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='alien'):
                self.stdout.write(f"  - {film.title} ({film.year})")
