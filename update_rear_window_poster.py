...
class Command(BaseCommand):
    help = 'Обновление постера для фильма "Окно во двор"'

    def handle(self, *args, **options):
        self.stdout.write("🪟 ОБНОВЛЕНИЕ ПОСТЕРА 'ОКНО ВО ДВОР'")
        self.stdout.write("=" * 50)
        
        # URL постера с Кино-Театр.ру
        poster_url = 'https://www.kino-teatr.ru/movie/posters/big/4/6/17064.jpg'
        
        try:
            # Ищем фильм
            film = Film.objects.get(title='Окно во двор')
            ...
            # Сохраняем постер
            filename = "rear_window_1954_kinoteatr.jpg"
            ...
            self.stdout.write(f"🎭 Режиссер: Альфред Хичкок")
                ...
                try:
                    with film.poster.open('rb') as f:
                        ...
                        if header.startswith(b'\xff\xd8\xff'):
                            ...
                            self.stdout.write(self.style.WARNING("⚠️ Возможно поврежден"))
            ...
            self.stdout.write(self.style.ERROR('❌ Фильм "Окно во двор" не найден'))
            self.stdout.write("📋 Поиск похожих фильмов:")
            for film in Film.objects.filter(title__icontains='окно'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='двор'):
                self.stdout.write(f"  - {film.title} ({film.year})")
            for film in Film.objects.filter(title__icontains='window'):
                self.stdout.write(f"  - {film.title} ({film.year})")
